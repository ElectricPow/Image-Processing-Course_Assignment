...
def save_selected_figures(filename, input_image, gt_image, outputs):
    ...
    save_color_image(FIGURES_DIR / f"cc_input_{stem}.png", input_image)
    save_color_image(FIGURES_DIR / f"cc_enhanced_{stem}.png", outputs["enhanced"])
    save_color_image(FIGURES_DIR / f"cc_corrected_{stem}.png", outputs["color_corrected"])
    save_color_image(FIGURES_DIR / f"cc_gt_{stem}.png", gt_image)

    enhanced_delta_ab = compute_delta_ab_map(outputs["enhanced"], gt_image)
    corrected_delta_ab = compute_delta_ab_map(outputs["color_corrected"], gt_image)
    save_color_image(
        FIGURES_DIR / f"cc_deltaab_enhanced_{stem}.png",
        build_heatmap_rgb(enhanced_delta_ab),
    )
    save_color_image(
        FIGURES_DIR / f"cc_deltaab_corrected_{stem}.png",
    ...
    save_color_image(FIGURES_DIR / f"cc_compare_enhanced_{stem}.png", enhanced_comparison)
    save_color_image(FIGURES_DIR / f"cc_compare_corrected_{stem}.png", corrected_comparison)
...
def main():
    ...
    save_csv(TABLES_DIR / "color_correction_guided_enhanced_metrics.csv", enhanced_rows)
    save_csv(TABLES_DIR / "color_correction_guided_corrected_metrics.csv", corrected_rows)
    save_csv(TABLES_DIR / "color_correction_selected_metrics.csv", selected_rows)

    average_rows = [
        {"variant": "guided_enhanced", **{k: f"{v:.4f}" for k, v in enhanced_average.items()}},
        {
            "variant": "guided_color_corrected",
            **{k: f"{v:.4f}" for k, v in corrected_average.items()},
        },
    ]
    save_csv(TABLES_DIR / "color_correction_average_metrics.csv", average_rows)

    average_df = pd.DataFrame(average_rows)
    average_df.to_csv(
        TABLES_DIR / "color_correction_average_metrics_pandas.csv",
        index=False,
        encoding="utf-8",
    )

    print("color correction analysis finished")
